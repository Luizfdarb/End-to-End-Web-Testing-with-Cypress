...
describe("New Transaction", function() {
  ...
  it("navigates to the new transaction form, selects a user and submits a transaction payment", function() {
    ...
    cy.getBySel("alert-bar-success").should("be.visible").and("have.text", "Transaction Submitted!");
    ...
    cy.database("find", "users", {
      id: ctx.contact!.id
    }).its("balance").should("equal", ctx.contact!.balance + parseInt(payment.amount) * 100);
  ...
  it("displays new transaction errors", function() {
    ...
    cy.get("#transaction-create-description-input-helper-text").should("be.visible").and("contain", "Please enter a note");
  ...
  it("submits a transaction payment and verifies the deposit for the receiver", function() {
    ...
    if (!isMobile()) {
      ...
      cy.get("[data-test=sidenav-user-balance]").invoke("text").then((x) => {
  ...
  it("submits a transaction request and accepts the request for the receiver", function() {
    ...
    cy.getBySelLike("transaction-item").first().should("contain", transactionPayload.description).click({
      force: true
    });
  ...
  it("searches for a user by attributes", function() {
    ...
    searchAttrs.forEach((attr: keyof User) => {
      ...
      cy.wait("@usersSearch");
      cy.getBySelLike("user-list-item").first().contains(targetUser[attr] as string);
